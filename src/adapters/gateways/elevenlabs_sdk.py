...
class ElevenLabsMusicGateway:
    ...
    async def compose_music_detailed(
        self,
        request: MusicGenerationRequest,
    ) -> tuple[MusicFile, dict[str, Any]]:
        """音楽を生成（詳細版）。

        コンポジションプランと音楽メタデータを含む詳細情報を返します。
        ...
        try:
            # 同期処理を非同期で実行
            loop = asyncio.get_event_loop()
            track_details = await loop.run_in_executor(
                None,
                lambda: client.music.compose_detailed(
                    prompt=request.build_prompt(),
                    music_length_ms=request.duration_ms,
                ),
            )
            ...
            # 音声データを取得
            audio_data = bytes(track_details.audio) if hasattr(track_details, "audio") else b""

            # MusicFileエンティティを作成
            music_file = MusicFile(
                file_name=track_details.filename
                if hasattr(track_details, "filename")
                else "generated_music.mp3",
                file_size_bytes=len(audio_data),
                duration_seconds=request.duration_seconds,
                format="mp3",
                data=audio_data,
            ...
            # 詳細情報を取得
            details = {}
            if hasattr(track_details, "json"):
                details = track_details.json

            self._logger.info(
                "詳細付き音楽生成が完了しました",
                extra={"has_composition_plan": "composition_plan" in details},
            )

            return music_file, details

        except Exception as e:
            self._logger.error(f"詳細付き音楽生成中にエラーが発生: {e}")
            raise AudioGenerationError(f"詳細付き音楽生成に失敗しました: {e}") from e
    ...
    async def stream_music(
        ...
        try:
            ...
            self._logger.info("音楽ストリーミングが完了しました")

        except Exception as e:
            self._logger.error(f"音楽ストリーミング中にエラーが発生: {e}")
            raise AudioGenerationError(f"音楽ストリーミングに失敗しました: {e}") from e
    ...
    async def create_composition_plan(
        ...
        プロンプトからより詳細な音楽生成計画を作成します。

        Args:
            prompt: 音楽生成用のプロンプト
            duration_ms: ミリ秒単位の長さ

        Returns:
            コンポジションプラン

        Raises:
            AudioGenerationError: プラン生成に失敗した場合
        """
        client = self._get_client()
        ...
        try:
            ...
            # CompositionPlanオブジェクトに変換
            plan = CompositionPlan(
                positive_global_styles=plan_data.get("positiveGlobalStyles", []),
                negative_global_styles=plan_data.get("negativeGlobalStyles", []),
                sections=plan_data.get("sections", []),
            )

            self._logger.info(
                "コンポジションプランを生成しました",
                extra={"sections_count": len(plan.sections)},
            )

            return plan

        except Exception as e:
            self._logger.error(f"コンポジションプラン生成中にエラーが発生: {e}")
            raise AudioGenerationError(f"コンポジションプラン生成に失敗しました: {e}") from e
